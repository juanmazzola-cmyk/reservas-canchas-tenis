...
<body class="bg-gray-100 min-h-screen pb-20">
    ...
    <!-- Bottom Nav -->
    <nav class="fixed bottom-0 left-0 right-0 bg-verde z-50 shadow-lg rounded-t-2xl overflow-x-auto">
    <div class="flex justify-around items-center h-16 px-1 min-w-full w-max text-gray-900">
        @auth
            @php
                $rol = auth()->user()->rol;
            ...
            @if($rol === 'usuario')
                ...
                @endif
                {{-- Carnet (solo socios, si está habilitado) --}}
                @if(auth()->user()->es_socio && $cfgUbic?->carnet_enabled)
                <a href="{{ route('carnet') }}" wire:navigate
                   class="flex flex-col items-center gap-0.5 text-xs px-2 py-1 rounded transition-colors {{ request()->routeIs('carnet') ? 'text-yellow-300 font-semibold' : 'text-gray-900 opacity-80 hover:opacity-100' }}">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        <path stroke-linecap="round" stroke-linejoin="round" d="M13 9h5M13 12h5M13 15h3"/>
                    </svg>
                    <span>Carnet</span>
                </a>
                @endif
            ...
            @if($rol === 'control')
                {{-- Escanear QR --}}
                <a href="{{ route('escanear-carnet') }}" wire:navigate
                   class="flex flex-col items-center gap-0.5 text-xs px-2 py-1 rounded transition-colors {{ request()->routeIs('escanear-carnet') ? 'text-yellow-300 font-semibold' : 'text-gray-900 opacity-80 hover:opacity-100' }}">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        <path stroke-linecap="round" stroke-linejoin="round" d="M13 15h1m2 0h1m-1-2v1"/>
                    </svg>
                    <span>Escanear</span>
                </a>
